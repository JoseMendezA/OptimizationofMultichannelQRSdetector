function fusion = performFusionOptTrain(det, detWin, coefficients, beta)
%
...
while ~all(emp)
    ...
    for i = 1 : length(count)
        if count(i) == 1 %If the lead signaled detection sum the corresponding ai
            sum = sum + coefficients(1,i+12); % coefficients from poblacion [L1,end-1)
        else %If the lead did not signal detection subtract the corresponding ai
            sum = sum - coefficients(1,i); % coefficients from poblacion [L1,end-1)
        end
    end
